...
@section('title', 'Loan Approval | ' . $organization->org_name)
@section('page_title', 'Loan Approval')
...
@include('layouts.client-info')
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control thousand-separator" id="application_amount"
                            name="application_amount" value="{{ number_format($loan->application_amount, 0, '.', ',') }}"
                            required disabled>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control thousand-separator" id="appraisal_amount"
                                    ...
                                    value="{{ number_format($loan->appraisal_amount, 0, '.', ',') }}" required disabled>
    ...
    <div class="col-sm-12">
        <div class="card card-outline card-success elevation-3">
            ...
            {{-- form --}}
            <form id="loanForm" method="post" action="{{ route('loans.approve', ['loan' => $loan]) }}"
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <select class="form-control select2" id="approval_officer_id"
                                        name="approval_officer_id" required>
                                        <option>--Select Approval Officer</option>
                                        @foreach ($users as $nowuser)
                                            <option value="{{ $nowuser->id }}"
                                                @if ($user->id === $nowuser->id) selected @endif>
                                                {{ $nowuser->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group date" id="reservationdate2" data-target-input="nearest">
                                    ...
                                    <input id="approved_date" name="approved_date" type="text"
                                        ...
                                        value="{{ old('approved_date') }}" placeholder="YYYY-MM-DD" required>
                                </div>
                                @error('approved_date')
                                    <div class="text-sm text-danger">{{ $message }}</div>
                                @enderror
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="text-sm m-0 p-o">Not greater than
                                    <b>{{ number_format($loan->appraisal_amount, 0, '.', ',') }} </b> on Appraisal
                                </div>
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text">{{ $organization->currency_code }}</span>
                                    ...
                                    <input type="text" class="form-control thousand-separator" id="approved_amount"
                                        name="approved_amount" value="{{ old('approved_amount') }}" required>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input type="number" class="form-control" id="approved_period"
                                        name="approved_period" value="{{ $loan->appraisal_period }}" required>
                                    <div class="input-group-append">
                                        <span class="input-group-text">Months</span>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="text-sm m-0 p-o">Adjust the Rate for this loan the default is
                                    {{ $loan->loanProduct->interest_rate }} %</div>
                                <div class="input-group">
                                    <input type="number" class="form-control" id="approved_interest_rate"
                                        name="approved_interest_rate" value="{{ $loan->loanProduct->interest_rate }}"
                                        required>
                                    <div class="input-group-append">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="approved_comment" name="approved_comment" class="form-control" rows="2"
                            placeholder="Enter Approval Comment" required></textarea>
                        @error('approved_comment')
                            <div class="text-sm text-danger">{{ $message }}</div>
                        @enderror
                    </div>

                    <input type="text" name="status" value="approved" required hidden>

                    <!-- /.card-body -->
                    <div class="card-footer">
                        <div class="card-tools text-right">
                            <button name="submit" type="submit" class="btn btn-success">Approve this Loan for
                                Disbursment</button>
                        </div>
                    </div>
            </form>
        ...
        <div class="card mt-5">
            ...
            <form method="post" action="{{ route('loans.defer', ['loan' => $loan]) }}"
                ...
                @method('PUT')
                    <div class="row">
                        <div class="col-md-12">
                            <p class="text-sm text-danger"><b>Defer this Loan</b><br>What is the reason for
                                Deferring this Loan Application?</p>
                                <div class="form-group">                                    
                                    <textarea id="defer_comment" name="defer_comment" class="form-control" rows="2"
                                        placeholder="Enter Deferment Comment" required></textarea>
                                    @error('defer_comment')
                                        <div class="text-sm text-danger">{{ $message }}</div>
                                    @enderror
                                </div>
                                <input type="text" name="status" value="deferred" required hidden>
                                <input type="text" name="approved_date" value=" {{ date('Y-m-d') }}" required hidden>
                                <input type="text" name="approval_officer_id" value="{{ $user->id }}" required hidden>
                               
                        </div>
                        <div class="col-md-12">
                            <div class="card-tools text-right">
                                <button name="submit" type="submit" class="btn btn-sm btn-danger text-sm">Defer This
                                    Loan</button>
                            </div>
