...
@section('main_content')
    ...
    <div class="col-sm-12">
        <form method="post" action="{{ route('settings.users.update', ['nowuser' => $nowuser]) }}">
            @csrf
            @method('PUT')
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="branch_id" name="branch_id" required>
                            ...
                            @foreach ($branches as $branch)
                                <option value="{{ $branch->id }}" @if ($nowuser->branch_id === $branch->id) selected @endif>
                                    {{ $branch->branch_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="name"
                            value="{{ $nowuser->name }}" placeholder="Enter name" required>
                        @error('name')
                            <div class="text-sm text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="user_name" name="user_name"
                            value="{{ $nowuser->user_name }}" placeholder="Enter username" required>
                        @error('user_name')
                            <div class="text-sm text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="user_phone" name="user_phone"
                                data-inputmask='"mask": "+256999999999"' data-mask value="{{ $nowuser->user_phone }}"
                                required>
                        </div>
                        @error('user_phone')
                            <div class="text-sm text-danger">{{ $message }}</div>
