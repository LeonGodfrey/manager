    ...
    <div class="col-12">
        <div class="form-group">
            <div role="group" class="btn-group">
                <ul class="nav">
                    <li>
                        <a href="{{ route('settings.users.index') }}" class="btn btn-success">Active Users</a>
                    </li>
                    <li>
                        <a href="{{ route('settings.users.disabled') }}" class="btn btn-default btn-success">Inactive</a>
                    </li>
                </ul>
            </div>
        </div>
                        ...
                        @unless ($users->isEmpty())
                            @foreach ($users as $nowuser)
                                <tr class="text-nowrap">
                                    <td><a
                                            href="{{ route('settings.users.edit', ['nowuser' => $nowuser]) }}">{{ $nowuser->name }}</a>
                                    </td>
                                    <td>{{ $nowuser->user_name }}</td>
                                    <td>{{ $nowuser->user_phone }}</td>
                                    <td>{{ $nowuser->email }}</td>
                                    <td>{{ $nowuser->branch->branch_name }}</td>
                                    <!-- Access the organization name via the relationship -->
                                    <td>user</td>
                                    <td>
                                        <form method="POST"
                                            action="{{ route('settings.users.disable', ['nowuser' => $nowuser->id]) }}">
                                            @csrf
                                            @method('PUT')
